...
uses Unitprogreso,unitglobalescol,unitdata, UnitQuerys,unitdatamodulo,
  UnitPrincipal,Unitglobales;

{$R *.dfm}

procedure TFrmReportes.aKeyPress(Sender: TObject;
  var Key: Char);
begin
        if Key = #13 then
        begin
          a.Value := frac(a.Value);
        end;
...
begin
        ...
        FrmProgreso.Caption := 'Espere Un Momento por Favor';
        FrmProgreso.Show;
        while not Eof do
        begin
        with IBQuery1 do begin
          ...
          SQL.Add('("col$colocacion".ID_TIPO_CUOTA < 4)');
...
procedure TFrmReportes.Button2Click(Sender: TObject);
...
begin
       ...
       // ConsecBarridoInicial := FormatCurr('00000000',ConteoConsec); consecutivo inicial del barrido empleados
        ...
        frmProgreso := TfrmProgreso.Create(self);
        frmProgreso.Caption := 'Efectuando Liquidaciones';
        frmProgreso.Barra.Minimum := 0;
        frmProgreso.Barra.Maximum := Total;
        frmProgreso.Barra.Position := 0;
        frmProgreso.Show;
...
//            vTotalAhorros := BuscoSaldoAhorros(FieldByName('IDIDENTIFICACION').AsInteger,FieldByName('IDPERSONA').AsString);
              ...
              for  I := 1 to CuotasPendientes do
               begin
                 ...
                 if vTotalLiquidacion <= vTotalAhorros then // descuento del sueldo
                  begin
                    abono_capital := 0;
                    abono_cxc := 0;
                    abono_servicios := 0;
                    ...
                    with IBExtracto do
                     begin
                       ...
                       ParamByName('ID_EMPLEADO').AsString := frmain.DBAlias;
                       ...
                       ParamByName('FECHA_PAGADA').AsDate := Date;
                       open;
                       IBExtracto.Close;
     ...
     //if StrToInt(ConsecBarridoFinal) - StrToInt(ConsecBarridoInicial)  > 0 then comprara consecutivos
     if 1 = 1 then
      begin
        with IBQTabla do
         begin
           ...
           SQL.Clear;
           SQL.Add('select ');
           SQL.Add('SUM("col$extractodet".VALOR_DEBITO) as TOTALDEBITO,');
           SQL.Add('SUM("col$extractodet".VALOR_CREDITO) as TOTALCREDITO');
           SQL.Add('from "col$extractodet"');
           SQL.Add('where');
           SQL.Add('("col$extractodet".ID_CBTE_COLOCACION >= :"ID1") and');
           SQL.Add('("col$extractodet".ID_CBTE_COLOCACION <= :"ID2")');
        ...
        VNocomprobante := FormatCurr('00000000',ObtenerConsecutivo(IBCuotas));
        ...
        with IBcomprobante do
         begin
           ...
           ParamByName('DESCRIPCION').AsString := 'Descuento de Cartera por Barrido de Fecha  ' + DateToStr(FechaHoy) + '  A las  ' + FormatDateTime('HH:MM:SS',Now);
...
function TFrmReportes.saldocredito(nit: integer): currency;
begin
