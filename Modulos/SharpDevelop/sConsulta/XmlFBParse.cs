...
//				System.Console.WriteLine("Cargando Xml...");
			...
			{
				CreateLogFiles.ErrorLog(globalvars.Logfile,_remoteip+":Error Cargando Xml..."+e.Message+"\n\r"+_receivestr);
			}
...
//				    System.Console.WriteLine("Sentencia\t\tConsulta[{0:00}]:"+sentencia,i);
					CreateLogFiles.ErrorLog(globalvars.Logfile,_remoteip+":ejecutando sentencia:"+sentencia);
					switch (ValidarTipo(tipo))
					{
							...
							break;
							case 4:		fExecute(i, sentencia);
							break;
							default:	fOthers(i, sentencia);
							break;
					}
				}
				catch (XmlException e)
				{
					CreateLogFiles.ErrorLog(globalvars.Logfile,_remoteip+":"+e.Message);
				}
...
//				System.Console.WriteLine("Transaction Iniciada...");
			...
			catch (FbException e)
			{
				...
				_consulta.AppendChild(_registro);
				_root.AppendChild(_consulta);
				CreateLogFiles.ErrorLog(globalvars.Logfile,"Error en el query:"+e.Message);				
				try{
					_reader.Close ();
				}
				catch
...
//				System.Console.WriteLine("Sentencia llega:{0}",sentencia);
			...
			while (true)
			{
				...
				{
					if (e.ErrorCode == -913) RecordLocked = true;
					Console.WriteLine(e.Message);
					if (RecordLocked)
					{
						Random random = new Random(20);
						WaitCount = random.Next();
					...
					else
					{
						_consulta.SetAttribute("campos","1");
						_consulta.SetAttribute("campotipo1","VARCHAR");
						_consulta.SetAttribute("camponombre1","ERROR_EN_EL_INSERT");
						_consulta.SetAttribute("campotamano1","200");
						_transaction.Rollback();
						CreateLogFiles.ErrorLog(globalvars.Logfile,"Error en el query:"+e.Message);						
						XmlElement _registro = _xmldocret.CreateElement("","Registro","");
...
//				System.Console.WriteLine("Sentencia llega:{0}",sentencia);
			...
			try
			{
				...
				_consulta.SetAttribute("campos","1");
				_consulta.SetAttribute("campotipo1","INTEGER");
				_consulta.SetAttribute("camponombre1","RESULTADO");
				_consulta.SetAttribute("campotamano1","0");
				XmlElement _registro = _xmldocret.CreateElement("","Registro","");
				XmlElement _campo = _xmldocret.CreateElement("","campo","");
				_campo.InnerText = Convert.ToString(_rows);
			...
			catch (FbException e)
			{
				_consulta.SetAttribute("campos","1");
				_consulta.SetAttribute("campotipo1","VARCHAR");
				_consulta.SetAttribute("camponombre1","ERROR EN EL EXECUTE");
				_consulta.SetAttribute("campotamano1","200");
				XmlElement _registro = _xmldocret.CreateElement("","Registro","");
				XmlElement _campo = _xmldocret.CreateElement("","campo","");
				_campo.InnerText = e.Message;
		...
		internal void fOthers(int i, string sentencia)
		{
			string ConnectionString;
			FbConnectionStringBuilder cs = new FbConnectionStringBuilder ();
			string _xmlelementstr = String.Format("{0}{1:000}","consulta",i);
...
//				System.Console.WriteLine("Sentencia llega:{0}",sentencia);
			...
			_consulta.SetAttribute("campos","1");
			_consulta.SetAttribute("campotipo1","VARCHAR");
			_consulta.SetAttribute("camponombre1","COMANDO EJECUTADO");
			_consulta.SetAttribute("campotamano1","50");
			XmlElement _registro = _xmldocret.CreateElement("","Registro","");
			XmlElement _campo = _xmldocret.CreateElement("","campo","");
			_campo.InnerText = "Correcto...";
			_registro.AppendChild(_campo);
			_consulta.AppendChild(_registro);
			_root.AppendChild(_consulta);
			}
			catch (FbException e)
			{
				_consulta.SetAttribute("campos","1");
				_consulta.SetAttribute("campotipo1","VARCHAR");
				_consulta.SetAttribute("camponombre1","ERROR EN EL COMANDO");
				_consulta.SetAttribute("campotamano1","200");
				XmlElement _registro = _xmldocret.CreateElement("","Registro","");
				XmlElement _campo = _xmldocret.CreateElement("","campo","");
				_campo.InnerText = e.Message;
