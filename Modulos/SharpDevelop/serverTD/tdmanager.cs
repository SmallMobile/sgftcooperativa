...
namespace serverTD
{
	...
	public class Tdmanager
	{
		...
		public void Execute()
		{
			...
			CreateLogFiles.ErrorLog(globalvars.Logfile,"Enviando Actualización a la Oficina...");
			
			XmlParse();
			
			CreateLogFiles.ErrorLog(globalvars.Logfile,"Recibiendo Respuesta de la Oficina...");
		...
		private void XmlParse()
		{
			...
			if (_xmlnode != null)
			{
				...
				string _net = _xmlnode.GetAttribute("NET");
				string _secuence = _xmlnode.GetAttribute("SECUENCE");
				...
				if (_net == "5"){
					globalvars._sAgencia _vDatafono = ValidarDatafono(_secuence.Substring(2,6));
					if (_vCard._agencia != _vDatafono._agencia){
						switch(_vDatafono._agencia){
								case 0: break;
								default:{
									_sOfcRetDatafono = UDPTransfer(_sOfcData, _vDatafono._host, _vDatafono._port, _returnPort);
									break;
								}
						}
					}
		...
		private globalvars._sAgencia ValidarDatafono(string _datafono)
		{
			string ConnectionString;
			FbConnectionStringBuilder cs = new FbConnectionStringBuilder ();
			FbDataReader _reader = null;
			...
			_qstr.Append("INNER JOIN GEN$SERVIDOR s ON (s.ID_AGENCIA = d.ID_AGENCIA) ");
			_qstr.Append("WHERE ");
			_qstr.Append("d.DATA_CODIGO = @DATA_CODIGO)");
			
			_query = new FbCommand(_qstr.ToString(),_conn,_tran);
			_query.Parameters.Add("@DATA_CODIGO",FbDbType.VarChar).Value = _datafono;
			try
			{
				_reader = _query.ExecuteReader();
				if (_reader.Read())
