...
interface
uses JvSimpleXml,Classes,DBClient,Db,SysUtils;
type
   TXml = class
     private
       _xXmlArchivo :TJvSimpleXML;
       _xNodo,_xNodo1 :TJvSimpleXmlElemClassic;
       _sTipo :string;
       _sSentencia: string;
...
begin
        _xXmlArchivo := TJvSimpleXML.Create(nil);
        _xNodo := _xXmlArchivo.Root.Items.Add('querys');
end;
procedure TXml.CargarNodo;
begin
        _xNodo1 := _xNodo.Items.Add('querys');
        _xNodo1.Items.Add('tipo',_sTipo);
        _xNodo1.Items.Add('sentencia',_sSentencia);
end;
...
var    _xXml :TJvSimpleXML;
       _xNod,_xNod1 :TJvSimpleXmlElemClassic;
...
//        _xNod := _xXml.Root.Items.Add(_sListaConsulta);
        _xNod := _xXml.Root.Items.Value(_sListaConsulta);
        try
          //_iCampos := _xNod.iteReadAttributeInteger('campos');
          _iCampos := _xNod.Properties.Count;
        except
          _iCampos := 0;
        end;
        for i := 1 to _iCampos do
        begin
          _sNombreCampo := _xNod.Properties.Value('camponombre' + IntToStr(j));
          _tTipoCampo := StrToFld(_xNod.Properties.Value('campotamano' + IntToStr(j)));
          _iTamano := StrToInt(_xNod.Properties.Value('campotipo' + IntToStr(j)));
