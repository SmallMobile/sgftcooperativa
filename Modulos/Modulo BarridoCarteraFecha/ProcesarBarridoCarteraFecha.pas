unit ProcesarBarridoCarteraFecha;

interface

uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms,
  Dialogs, StdCtrls, ComCtrls, DB, IBCustomDataSet, IBQuery, DBClient,
  IBSQL, IBDatabase,DateUtils,Math,IniFiles;
...
var
  MiIni:TIniFile;
begin
  MiIni := TIniFile.Create('./BarridoCarteraFecha.ini');
  DBName := MiIni.ReadString('DBSERVER','DBName','192.168.200.7:/var/db/fbird/database.fdb');
  DBAlias := MiIni.ReadString('DBSERVER','DBUser','BARRIDO');
  DBPasabordo := MiIni.ReadString('DBSERVER','DBPassword','barrido');
  Agencia := StrToInt(MiIni.ReadString('DBSERVER','Agencia','1'));

  dmGeneral.IBDatabase1.DataBaseName := DBname;
  ...
  dmGeneral.IBDatabase1.Params.Values['sql_role_name'] := 'CARTERA';
  try
...
procedure TfrmBarridoCartera.Procesar;
var Fecha:TDate;
...
begin
        ...
        with IBQuery1 do begin
          ...
          try
            ...
            begin
               Close;
               Label2.Caption := 'No se encontraron registros!';
               Exit;
            end
            else
            begin
              Label4.Caption := 'Procesando Colocaciones';
              while not Eof do
               begin
                Bar1.Position := RecNo;
                Label1.Caption := 'Colocación No:' + FieldByName('ID_COLOCACION').AsString;
            ...
            Close;
            label2.Caption := 'Error al buscar colocaciones, consulte con sistemas';
            Exit;
           end;
        end;
        if DSBarrido.RecordCount > 0 then
          ...
          Label2.Caption := 'No se Encontraron Colocaciones Vencidas a la Fecha';
end;

procedure TfrmBarridoCartera.Ejecutar;
...
begin
        ...
        Label4.Caption := 'Efectuando Liquidaciones';
        Bar1.Min := 0;
        Bar1.Max := Total;
        Bar1.Position := 0;
         ...
         begin
           ...
           while not Eof do
            begin
              Bar1.Position := RecNo;
              Label1.Caption := 'Colocación No:' + FieldByName('COLOCACION').AsString + '-' + IntToStr(RecNo);
              Application.ProcessMessages;
              ...
              while I <= CuotasPendientes do
               begin
                 ...
                 if FieldByName('TIPOCUOTA').AsInteger = 1 then
                    LiquidarCuotasFija(Agencia,Colocacion,1,FechaHoy,
                    ...
                    LiquidarCuotasVarAnticipada(Agencia,Colocacion,1,FechaHoy,
                    CuotasLiq,Clasificacion,Garantia,Categoria,Estado,ValorCuota,
                    FechaPagoK,FechaPagoI,TipoInteres,ValorTasa,ValorMora,PuntosAdic,SaldoActual,AmortizaK,AmortizaI,DiasProrroga)
                 else if FieldByName('TIPOCUOTA').AsInteger = 3 then
                    LiquidarCuotasVarVencida(Agencia,Colocacion,1,FechaHoy,
                    CuotasLiq,Clasificacion,Garantia,Categoria,Estado,ValorCuota,
                    FechaPagoK,FechaPagoI,TipoInteres,ValorTasa,ValorMora,PuntosAdic,SaldoActual,AmortizaK,AmortizaI,DiasProrroga);
     ...
     if ContadorLiq > 0 then
      begin
        ...
        with IBcomprobante do
         begin
           ...
           ParamByName('DESCRIPCION').AsString := 'Descuento de Cartera por Barrido de Fecha ' + DateToStr(FechaHoy) + ' A las ' + FormatDateTime('T',fHoraActual);
      ...
      begin
        Label2.Caption := 'No se Realizaron Liquidaciones por Insuficiencia de Ahorros';
...
procedure TfrmBarridoCartera.BtnProcesarClick(Sender: TObject);
begin
  FechaHoy := Fecha.Date;
  Procesar;
end;
