...
begin
        ...
        begin
          ...
          Transaction.StartTransaction;
          SQL.Clear;
          SQL.Add('select * from "cap$causacioncon" where MES = :MES');
          ...
          Close;
          SQL.Clear;
          SQL.Add('select * from P_CAP_CON(5,1,' + '''' + DateToStr(Dfecha.Date) + '''' + ' )');
          Open;
          Last;
          First;
          ...
          begin
            ...
            //if FieldByName('numero_cuenta').AsInteger = 1389 then
               //ShowMessage('listo');
            ...
            CdContractual.FieldValues['dias'] := vContractual(FieldByName('FECHA_APERTURA').AsDateTime,FieldByName('FECHA_VENCIMIENTO').AsDateTime,FieldByName('PLAZO').AsInteger,FieldByName('CUOTA').AsCurrency,FieldByName('VALOR_INICIAL').AsCurrency).Dias;
            CdContractual.FieldValues['causacion_mensual'] := vContractual(FieldByName('FECHA_APERTURA').AsDateTime,FieldByName('FECHA_VENCIMIENTO').AsDateTime,FieldByName('PLAZO').AsInteger,FieldByName('CUOTA').AsCurrency,FieldByName('VALOR_INICIAL').AsCurrency).CausacionMensual;
            CdContractual.FieldValues['causacion_acumulada'] := vContractual(FieldByName('FECHA_APERTURA').AsDateTime,FieldByName('FECHA_VENCIMIENTO').AsDateTime,FieldByName('PLAZO').AsInteger,FieldByName('CUOTA').AsCurrency,FieldByName('VALOR_INICIAL').AsCurrency).CausacionAcumulada;
            CdContractual.FieldValues['retefuente_mensual'] := vContractual(FieldByName('FECHA_APERTURA').AsDateTime,FieldByName('FECHA_VENCIMIENTO').AsDateTime,FieldByName('PLAZO').AsInteger,FieldByName('CUOTA').AsCurrency,FieldByName('VALOR_INICIAL').AsCurrency).RetefuenteMensual;
            CdContractual.FieldValues['retefuente_acumulada'] := vContractual(FieldByName('FECHA_APERTURA').AsDateTime,FieldByName('FECHA_VENCIMIENTO').AsDateTime,FieldByName('PLAZO').AsInteger,FieldByName('CUOTA').AsCurrency,FieldByName('VALOR_INICIAL').AsCurrency).RetefuenteAcumulada;
            CdContractual.Post;
            except
            on e: Exception do
            begin
...
begin
       ...
       // if SeAcumula then
        ...
        if (Vcausacion >= 1100) and (vAplicaRetefuente) then
        begin
          if SeAcumula then
       ...
       { if (AnoVencimiento < AnoActual) then
        ...
        begin}
        if ((AnoVencimiento <= AnoActual) and (MesVencimiento <= MesActual)) or (AnoVencimiento < AnoActual) then
        begin
