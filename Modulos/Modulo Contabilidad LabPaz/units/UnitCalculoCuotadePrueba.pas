...
{                      if FieldByName('TASA_INTERES_CORRIENTE').AsFloat < FieldByName('VALOR_ACTUAL_TASA').AsFloat then
                      ...
                      EdTasaMaxima.Caption := FormatCurr('#0.00%',BuscoTasaEfectivaMaxima(fFechaActual));
                      ...
                      begin
                         EdTasaMaximaNominal.Caption := FormatCurr('#0.00%',TasaNominalVencida(BuscoTasaEfectivaMaxima(fFechaActual),FieldByName('AMORTIZA_INTERES').AsInteger));
                      ...
                      begin
                         EdTasaMaximaNominal.Caption := FormatCurr('#0.00%',TasaNominalAnticipada(BuscoTasaEfectivaMaxima(fFechaActual),FieldByName('AMORTIZA_INTERES').AsInteger));
...
//          CuotasLiq.Lista.Pack;
     ...
     if (AR^.Debito <> 0) or (AR^.Credito <> 0) then CuotasLiq.Lista.Add(AR);
          ...
          if (vNacional = 0) and ((CredFallecido = False) and (CredIncapacitado = False)) then
            begin
...
//                  AF^.CodigoPuc := CodigoBanco;
     ...
     if (not CredFallecido) and (not CredIncapacitado) then begin
      if vTotalLiquidacion > (vTotalAhorros + (vNacional - vComisionNal)) then
       begin
