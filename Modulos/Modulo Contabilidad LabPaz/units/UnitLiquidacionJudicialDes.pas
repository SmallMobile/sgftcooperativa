     ...
     try
      ...
      if  MyCuotasLiq.Lista.Count > 0 then
       ...
       if vhonorarios > 0 then begin
         SQL.Clear;
         SQL.Add('INSERT INTO "col$pagohonorariostmp"');
         SQL.Add('values(:ID_AGENCIA,:ID_COLOCACION,:ID_COLOCACION_DES,:ID_SOLICITUD,');
         SQL.Add(':NUMERO_CUENTA_AB,:DIGITO_CUENTA_AB,:VALOR,:ID_COMPROBANTE)');
         ParamByName('ID_AGENCIA').AsInteger := Agencia;
         ParamByName('ID_COLOCACION').AsString := vColocacion;
         ParamByName('ID_COLOCACION_DES').AsString := '';
         ParamByName('ID_SOLICITUD').AsString := vIdSolicitud;
         ...
         ParamByName('VALOR').AsCurrency := vhonorarios;
...
procedure TfrmLiquidacionJudicialDes.EdTasaAplicadaExit(Sender: TObject);
begin
        if EdTasaAplicada.Text = '' then EdTasaAplicada.Text := '0.00';
        vTasaCas := StrToFloat(EdTasaAplicada.Text);
