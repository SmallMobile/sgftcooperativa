       ...
       if dmColocacion.IBQuery1.RecordCount > 0 then
        begin
          ...
          if FieldByName('INTERES').AsString = 'V' then
           begin
             EdTasaMax.Text := FormatCurr('#0.00%',TasaNominalVencida(BuscoTasaEfectivaMaxima(Date),FieldByName('AMORTIZA_INTERES').AsInteger));
           ...
           begin
             EdTasaMax.Text := FormatCurr('#0.00%',TasaNominalAnticipada(BuscoTasaEfectivaMaxima(Date),FieldByName('AMORTIZA_INTERES').AsInteger));
