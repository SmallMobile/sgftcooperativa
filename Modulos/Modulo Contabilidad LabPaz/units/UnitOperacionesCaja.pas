...
uses
  ...
  IdComponent, IdTCPConnection, IdTCPClient, JclSysUtils, Math, IBDatabase;
...
type
  TfrmOperacionesCaja = class(TForm)
    ...
    procedure EdTasaAplicadaCasExit(Sender: TObject);
...
begin
        ...
        if CapOperacion = 'R' then
         if (VGeneral > Disponible) and (VGeneral > Saldo) then
          begin
...
// Caso Contractual, Marcar Tabla
              if TipoCaptacion = 5 then begin
                IBPagar.Close;
                IBPagar.SQL.Clear;
                IBPagar.SQL.Add('select first 1 * from "cap$tablaliquidacioncon"');
                IBPagar.SQL.Add('where ID_AGENCIA = :ID_AGENCIA and ID_TIPO_CAPTACION = :ID_TIPO_CAPTACION and');
                IBPagar.SQL.Add('NUMERO_CUENTA = :NUMERO_CUENTA and DIGITO_CUENTA = :DIGITO_CUENTA and');
                IBPagar.SQL.Add('DESCONTADO = 0 order by FECHA_DESCUENTO');
                IBPagar.ParamByName('ID_AGENCIA').AsInteger := Agencia;
                IBPagar.ParamByName('ID_TIPO_CAPTACION').AsInteger := TipoCaptacion;
                IBPagar.ParamByName('NUMERO_CUENTA').AsInteger := StrToInt(EdNumeroCap.Text);
                IBPagar.ParamByName('DIGITO_CUENTA').AsInteger := StrToInt(EdDigitoCap.Caption);
                try
                  ...
                  IBPagar.Transaction.RollbackRetaining;
                  MessageDlg('Error Buscando Fecha a Marcar',mtError,[mbcancel],0);
                  Exit;
                end;

                FechaTabla := IBPagar.FieldByName('FECHA_DESCUENTO').AsDate;

                IBPagar.Close;
                IBPagar.SQL.Clear;
                IBPagar.SQL.Add('update "cap$tablaliquidacioncon" set DESCONTADO = 1');
                IBPagar.SQL.Add('where ID_AGENCIA = :ID_AGENCIA and ID_TIPO_CAPTACION = :ID_TIPO_CAPTACION and');
                IBPagar.SQL.Add('NUMERO_CUENTA = :NUMERO_CUENTA and DIGITO_CUENTA = :DIGITO_CUENTA and');
                IBPagar.SQL.Add('FECHA_DESCUENTO = :FECHA');
                IBPagar.ParamByName('ID_AGENCIA').AsInteger := Agencia;
                IBPagar.ParamByName('ID_TIPO_CAPTACION').AsInteger := TipoCaptacion;
                IBPagar.ParamByName('NUMERO_CUENTA').AsInteger := StrToInt(EdNumeroCap.Text);
                IBPagar.ParamByName('DIGITO_CUENTA').AsInteger := StrToInt(EdDigitoCap.Caption);
                IBPagar.ParamByName('FECHA').AsDate := FechaTabla;
                try
                  IBPagar.ExecQuery;
                except
                  IBPagar.Transaction.RollbackRetaining;
                  MessageDlg('Error al Actualizar Tabla',mtError,[mbcancel],0);
                  Exit;
                end;
              end;
//Fin Caso Contractual
              ...
              IBOtros.SQL.Add(':DOCUMENTO,:CHEQUES_MOVIMIENTO,:BILLETES,:MONEDAS,:CHEQUES,:HUELLA)');
              IBOtros.ParamByName('ID_AGENCIA').AsInteger := Agencia;
              IBOtros.ParamByName('ID_CAJA').AsInteger := StrToInt(EdCaja.Caption);
...
procedure TfrmOperacionesCaja.EdTasaAplicadaContingenteExit(
  Sender: TObject);
begin
...
procedure TfrmOperacionesCaja.EdTasaDevolucionExit(Sender: TObject);
begin
...
procedure TfrmOperacionesCaja.EdTasaCobradoCausadoExit(Sender: TObject);
begin
...
procedure TfrmOperacionesCaja.EdTasaCobradoCorrienteExit(Sender: TObject);
begin
...
//          m_data := VarArrayCreate([0,frmRegister.blobSize],varByte);
          m_data := VarArrayCreate([0,617],varByte);
...
begin
          ...
          TGeneralCas := vAbonoCapitalCas + vAbonoInteresCas + vAbonoCostasCas + vAbonoHonorariosCas + vPapeleria;
...
procedure TfrmOperacionesCaja.EdTasaAplicadaCasExit(Sender: TObject);
begin
        if EdTasaAplicadaCas.Text = '' then EdTasaAplicadaCas.Text := '0.00';
        vTasaAplicadaCas := StrToFloat(EdTasaAplicadaCas.Text);
...
begin
    if MessageDlg('Seguro de Realizar el Abono?',mtConfirmation,[mbYes,mbNo],0) = mrYes then
     begin
              ...
              IBOtros.ParamByName('OTROS').AsCurrency := vAbonoCostasCas + vAbonoHonorariosCas + vPapeleria;
        ...
        Timer.Enabled := False;
           ...
           begin
               ...
               CValidar.Devuelto := 0;
               CValidar.Otras := vAbonoCostasCas + vAbonoHonorariosCas + vPapeleria;
        ...
        begin
         ...
         begin
           ...
           begin
             ...
             begin
               CBTitularesAg.Items.Add(nodo.Nodes[i].ReadString('nombre'));
               if CapOperacion = 'R' then
               begin
                 ...
                 begin
                   ...
                   CDhuella.FieldValues['titular'] := nodo.Nodes[i].ReadString('nombre');
                   CDhuella.Post;
                 end
                 else
                 begin
                   ...
                   CDfirma.FieldValues['titular'] := nodo.Nodes[i].ReadString('nombre');
                   CDfirma.Post;
                 end;
               end;
             end;
...
begin
        ...
        begin
              ...
              IBOtros.SQL.Add(':ID_CAJA,:FECHA_MOV,:ID_AGENCIA,:ID_TIPO_CAPTACION,');
              IBOtros.SQL.Add(':NUMERO_CUENTA,:DIGITO_CUENTA,:ORIGEN_MOVIMIENTO,:ID_TIPO_MOVIMIENTO,');
              IBOtros.SQL.Add(':DOCUMENTO,:CHEQUES_MOVIMIENTO,:BILLETES,:MONEDAS,:CHEQUES,:ID_AGENCIA_REMOTA,:HUELLA,:ASOCIADO,:ES_CREDITO,0,:ID_PERSONA,:ID_IDENTIFICACION)');
              IBOtros.ParamByName('ID_AGENCIA').AsInteger := Agencia;
              IBOtros.ParamByName('ID_CAJA').AsInteger := StrToInt(EdCaja.Caption);
...
//**** REGISTRO DEL VALOR DE LA COMISION
              {if Vcomision > 0 then
              begin
                DigitoAportes := StrToInt(DigitoControl(1,EdNumeroCapAg.Text));
                IBOtros.Close;
                IBOtros.SQL.Clear;
                IBOtros.SQL.Add('insert into "caj$movremotosalida" values(');
                IBOtros.SQL.Add(':ID_CAJA,:FECHA_MOV,:ID_AGENCIA,:ID_TIPO_CAPTACION,');
                ...
                IBOtros.ParamByName('ID_CAJA').AsInteger := StrToInt(EdCaja.Caption);
                IBOtros.ParamByName('FECHA_MOV').AsDateTime := StrToDateTime(DateToStr(vFecha_Sucursal) + ' ' + TimeToStr(vHora_Sucursal));
                IBOtros.ParamByName('ID_TIPO_CAPTACION').AsInteger := 1;
                IBOtros.ParamByName('NUMERO_CUENTA').AsInteger := C_Aportes;
                IBOtros.ParamByName('DIGITO_CUENTA').AsInteger := D_Aportes;
                IBOtros.ParamByName('ORIGEN_MOVIMIENTO').AsInteger := 1;
                IBOtros.ParamByName('ID_TIPO_MOVIMIENTO').AsInteger := 1;
                IBOtros.ParamByName('DOCUMENTO').AsString := EdDocumentoCapag.Text;
                IBOtros.ParamByName('CHEQUES_MOVIMIENTO').AsInteger := ListaCheques.Count;
                ...
                IBOtros.ParamByName('HUELLA').AsInteger := 0;
                IBOtros.ParamByName('ASOCIADO').AsString := Asociado_Titular;
                IBOtros.ParamByName('ES_CREDITO').AsInteger := 0;//BoolToInt(ChkCreditos.Checked);
                try
                  IBOtros.ExecQuery;
                except
                on e: Exception do
...
var    Porcentaje,Maximo,Minimo,Comision :Currency;
begin
        with IBOtros do
        begin
           Close;
           SQL.Clear;
           SQL.Add('select * from "gen$minimos" where ID_MINIMO in (13,14,15)');
           ExecQuery;
           while not Eof do
           begin
...
begin
        ...
        FrmRemotaConsulta.Host_remoto := HostServidor;
