...
uses StrUtils,SysUtils,Math,Classes,DateUtils,SqlExpr,DBXpress,Variants;
...
var I, WaitCount, Tries,Consecutivo:Integer;
    ...
    ErrorMsg:string;
    IBSConsecBar:TSQLQuery;
    IDT1:TTransactionDesc;
begin
       Result := 0;
       Tries := 0;
       while True do begin
//       IDT1.TransactionID := 1+Random(100);
//       IDT1.IsolationLevel := xilREADCOMMITTED;

       IBSConsecBar := TSQLQuery.Create(nil);
       IBSConsecBar.SQLConnection := ibdatabase1;


        with IBSConsecBar do begin
         Close;
...
var I, WaitCount, Tries,Consecutivo:Integer;
    RecordLocked:Boolean;
    ErrorMsg:string;
    IBSConsecComp:TSQLQuery;
    IDT2:TTransactionDesc;
begin
       Result := 0;
       Tries := 0;
       while True do begin
//       IDT2.TransactionID := 1+Random(100);
//       IDT2.IsolationLevel := xilREADCOMMITTED;

       IBSConsecComp := TSQLQuery.Create(nil);
       IBSConsecComp.SQLConnection := ibdatabase1;

        with IBSConsecComp do begin
         Close;
...
begin
        ...
        begin
                ...
                Open;
                TasaEfect := FieldByName('VALOR_TASA_EFECTIVA').AsFloat;
                if (TasaEfect = 0) or (varisnull(TasaEfect))  then
                begin
                        Existe := True;
                        close;
                        SQL.Clear;
                        SQL.Add('select VALOR_TASA_EFECTIVA from ');
                        SQL.Add('"col$tasafijas" order by FECHA_INICIAL DESC ');
                        try
                          Open;
...
IBSQLTmp :TSQLQuery;
IDT3:TTransactionDesc;
begin
//     IDT3.TransactionID := 1+ Random(100);
//     IDT3.IsolationLevel := xilREADCOMMITTED;
     ...
     IBSQLTmp.SQLConnection := ibdatabase1;
      ...
      with IBSQLTmp do
       begin
//         IBDatabase1.StartTransaction(IDT3);
