...
uses IniFiles,SysUtils,QControls,DateUtils,StrUtils,Math,SqlExpr,DBXpress,
...
var
  MiIni:TIniFile;
begin
  MiIni := TIniFile.Create('/usr/bin/Barrido.ini');
  ...
  IBQTabla1 := TSQLDataSet.Create(nil);
  ...
  IBDatabase1.ConnectionName := 'IBConnection';
  ...
  IBDatabase1.LibraryName := 'libsqlib.so.1';
  IBDatabase1.LoginPrompt := False;
  IBDatabase1.VendorLib := 'libgds.so.0';
  try
   IBDatabase1.Open;
  except
   raise;
...
begin
       IDT.TransactionID := Random(1000)+1;
       IDT.IsolationLevel := xilREADCOMMITTED;
       with IBQTabla1 do
       begin
         IBDatabase1.StartTransaction(IDT);
         CommandType := ctQuery;
         CommandText := 'delete from "col$barridoCarteraAut"';
//         SQL.Clear;
//         SQL.Add('delete from "col$barridoCarteraAut"');
         ExecSQL;
