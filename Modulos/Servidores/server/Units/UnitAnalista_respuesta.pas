...
uses
  ...
  IdTCPServer, sdXmlDocuments ,
  DBXpress, SqlExpr, FMTBcd, DateUtils, QExtCtrls, QDBCtrls, DB;
type
       ...
       try
        ...
        begin
            ...
            begin
              ...
              system.Writeln(archivo,'Solicitud = ' + id_solicitud + ' Tipo = ' + id_tipo + ' Modulo = ' + id_modulo + ' Fecha y Hora : ' + FormatDateTime('yyyy/mm/dd - hh:mm:ss',Now) + ' Estado = No Encontrado');
            ...
            begin
              ...
              system.Append(archivo);
              system.Writeln(archivo,'Solicitud = ' + id_solicitud + ' Tipo = ' + id_tipo + ' Modulo = ' + id_modulo + ' Fecha y Hora : ' + FormatDateTime('yyyy/mm/dd - hh:mm:ss',Now) + ' Estado = Encontrado');
              system.Flush(archivo);
              system.CloseFile(archivo);
              nodo := XmlRes.Root.NodeNew('solicitud');
            ...
            //Inicio Datos Bancoldex
            nodo := xmlres.Root.NodeNew('bancoldex');
            SQL.Clear;
            ...
            while not Eof do
             begin
               nodo.WriteString('NO_BANCOLDEX',FieldByName('NO_BANCOLDEX').AsString);
               nodo.WriteString('EMPLEOS_ACT',FieldByName('EMPLEOS_ACT').AsString);
               nodo.WriteString('EMPLEOS_GENERAR',FieldByName('EMPLEOS_GENERAR').AsString);
               nodo.WriteString('TIPO_SOCIEDAD',FieldByName('TIPO_SOCIEDAD').AsInteger);
               nodo.WriteString('DESTINO1',FieldByName('DESTINO1').AsInteger);
               nodo.WriteString('DESTINO2',FieldByName('DESTINO2').AsInteger);
               nodo.WriteString('DESTINO3',FieldByName('DESTINO3').AsInteger);
               nodo.WriteString('MONTO1',FieldByName('MONTO1').AsCurrency);
               nodo.WriteString('MONTO2',FieldByName('MONTO2').AsCurrency);
               nodo.WriteString('MONTO3',FieldByName('MONTO3').AsCurrency);
               nodo.WriteString('GARANTIA1',FieldByName('GARANTIA1').AsInteger);
               nodo.WriteString('GARANTIA2',FieldByName('GARANTIA2').AsInteger);
               nodo.WriteString('GARANTIA2',FieldByName('GARANTIA3').AsInteger);
               nodo.WriteString('VALORGAR1',FieldByName('VALORGAR1').AsCurrency);
               nodo.WriteString('VALORGAR2',FieldByName('VALORGAR2').AsCurrency);
               nodo.WriteString('VALORGAR3',FieldByName('VALORGAR3').AsCurrency);
               nodo.WriteString('TOTAL_ACTIVOS',FieldByName('TOTAL_ACTIVOS').AsCurrency);
               nodo.WriteString('FECHA_CORTE_ACTIVOS',FieldByName('FECHA_CORTE_ACTIVOS').AsString);
               nodo.WriteString('LINEA_BANCOLDEX',FieldByName('LINEA_BANCOLDEX').AsString);
               nodo.WriteString('VISITA',FieldByName('VISITA').AsInteger);
               nodo.WriteDateTime('FECHA_VISITA',FieldByName('FECHA_VISITA').AsDateTime);
               Next;
             end;
            // Fin Datos Bancoldex
            end; // fin del valida solcitud
         ...
         begin
           ...
           try
             ...
             SQL.Add('"col$solicitud".INVERSION = :INVERSION, "col$solicitud".CLASIFICACION = :CLASIFICACION,"col$solicitud".FECHA_ANALISIS = :FECHA_ANALISIS');
             SQL.Add('where ID_SOLICITUD = :ID_SOLICITUD');
             ParamByName('ID_SOLICITUD').AsString := id_solicitud;
             ...
             ExecSQL;
             if nodo.ReadInteger('visita') = 1 then
              begin
                SQL.Clear;
                ...
                ParamByName('FECHA').AsDate := nodo.ReadDateTime('fechavisita');
                ExecQuery;
              end;
            ...
            try
             ...
             try
               ...
               begin
                 ...
                 Close;
                 SQL.Clear;
                 SQL.Add('insert into "col$desembolsoparcial"(ID_SOLICITUD,ID_DESEMBOLSO,DESEMBOLSADO,ID_AGENCIA) values (');
                 SQL.Add(':ID_SOLICITUD,1,0,:ID_AGENCIA)');
                 ParamByName('ID_SOLICITUD').AsString := id_solicitud;
...
//                 ParamByName('DESEMBOLSADO').AsInteger :=0;
      ...
      //Dispose(ARecord);
        ...
        CompressStream(Astream,Astream1);
        Astream1.Seek(0,0);
...
//     AssignFile(archivo,'/home/sistemas/analistad.log');
//     Archivo_respaldo := '/home/sistemas/analistad.log';
