...
begin
        ...
        PrimerConsec := False;
        ContadorLiq := 0;
//        ConteoConsec := ObtenerConsecutivoBarrido(IBCuotas) - 1;
...
//           ParamByName('ID2').AsInteger := StrToInt(ConsecBarridoFinal);
        ...
        with IBcomprobante do
         begin
           ...
           ParamByName('DESCRIPCION').AsString := 'Descuento de Cartera por Barrido de Fecha ' + DateToStr(FechaHoy) + ' A las ' + FormatDateTime('HH:MM:SS',fHoraActual);
...
function TfrmBarridoCartera.BuscoSaldoAhorros(vtipoid:Integer;vNumeroId:string) : Currency;
begin
  with IBSaldo do
   begin
     if Transaction.InTransaction then
     ...
     try
      ...
      if RecordCount < 1 then
        ...
        SaldoAhorros := FieldByName('SALDO_ACTUAL').AsCurrency;
     except
     ...
     Result := SaldoAhorros - TotalCanje;
   end;
end;
        ...
        EdImpresora.Caption := PrinterName;
         with IBGrupo1 do
          begin
           ...
           sql.Add('and "col$extractodet".FECHA_EXTRACTO = :"FECHA_EXTRACTO"');
...
{           while not IBGrupo1.Eof do
            begin
              if imprimir = False then
               begin
                 ImprimirCadena(PrinterName,Empresa + '   ' + 'Nit : ' + Nit + '                              ' + 'LIQUIDACION DE CARTERA' + '                             ' + DateToStr(Date)+ #13 +#10);
                 ImprimirCadena(PrinterName,'COLOCACION  '+ '   ' + 'ASOCIADO' + '                     ' + 'COMP. No. ' + 'CUENTA    ' + 'FECHA LIQUIDACION' + 'CAPITAL HASTA' + 'INTERES HASTA' +'SALDO ANTERIOR' + #13 +#10);
                 ImprimirCadena(PrinterName,Format('%.15s',[FieldByName('ID_COLOCACION').AsString]) + Format('%.40s',[FieldByName('NOMBRE').AsString + ' ' + FieldByName('PRIMER_APELLIDO').AsString + '  ' + FieldByName('SEGUNDO_APELLIDO').AsString]) + Format('%.8s',[IntToStr(FieldByName('ID_CBTE_COLOCACION').AsInteger)]) + Format('%.7s',[Cuenta]) + Format('%.15s',[DateToStr(FieldByName('FECHA_EXTRACTO').AsDateTime)]) + Format('%.15s',[DateToStr(FieldByName('CAPITAL_PAGO_HASTA').AsDateTime)]) + Format('%.15s',[DateToStr(FieldByName('INTERES_PAGO_HASTA').AsDateTime)]) + Format('%18.2m',[FieldByName('SALDO_ANTERIOR_EXTRACTO').AsCurrency]) + #13 + #10);
                 ImprimirCadena(PrinterName,'------------------------------------------------------------------------------------------------------------------------------------' + #13 +#10);
                 ImprimirCadena(PrinterName,'CUOTA' + '      CODIGO      ' + '|' + '     CUENTA CONTABLE     ' + '|' + 'FECHA INIC' + '|' + 'FECHA FIN' + '|' + 'DIAS' + '|' +'TASA ' + '|' + '      DEBITO      ' + '|' + '     CREDITO      ' + #13 + #10);
                 ImprimirCadena(PrinterName,'------------------------------------------------------------------------------------------------------------------------------------' + #13 + #10);
                 imprimir := True;
               end;
              ImprimirCadena(PrinterName,format('%.5s',[IntToStr(FieldByName('CUOTA_EXTRACTO').AsInteger)]) + '|' + Format('%.20s',[FieldByName('CODIGO_PUC').AsString]) + '|' + Format('%.25s',[FieldByName('CUENTA').AsString]) + '|' + Format('%.10s',[FieldByName('FECHA_INICIAL').asDate]) + '|' + Format('%.10s',[FieldByName('FECHA_FINAL').AsDate]) + '|' +
