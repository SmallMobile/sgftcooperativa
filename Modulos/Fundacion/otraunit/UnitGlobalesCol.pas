...
Uses Dialogs,Forms,StdCtrls,DBCtrls,IB, IBSQL, IBQuery,
...
//               TasaMora := ValorMora;
            ...
            if AF^.Devuelto then
             begin
               if TipoInteres = 0 then
                begin
                  TasaDev := BuscoTasaEfectivaMaxima(AF^.FechaInicial);
                  if ValorTasa > TasaDev then
                     TasaLiquidar := TasaDev
                  else
                     TasaLiquidar := ValorTasa;
                end
               else if TipoInteres = 1 then TasaDev := BuscoTasaEfectivaMaximaDtf(AF^.FechaInicial)
               else TasaDev := BuscoTasaEfectivaMaximaIPC;
             end;
...
begin
  ...
  with dmColocacion.IBQuery do
   begin
     ...
     SQL.Add('ID_AGENCIA = :"ID_AGENCIA" and ID_COLOCACION = :"ID_COLOCACION" and PAGADA = 0 and PAGADA = 0 and FECHA_A_PAGAR < :"FECHA_CORTE"');
     ParamByName('ID_AGENCIA').AsInteger := IdAgencia;
     ParamByName('ID_COLOCACION').AsString := IdColocacion;
      ...
      begin
        ...
        CalcularFechasLiquidarFija(FechaArranque,FechaCorte,FechaProx,FechasLiq);
         if FechaPagoI > FechaCorte then
          ...
          begin
            New(AF);
            AF^.FechaInicial := FechaInicialMora;
         ...
         for J := 0 to (FechasLiq.Count - 1) do
          begin
            ...
            if ((Int(FechaArranque) = Int(AR^.FechaInicial)) and (Mes=3) and (Dia=1) and (AF^.Devuelto=False) and (AF^.Vencida=False)) then
               ...
               Bisiesto := False;
//             AR^.Dias := DiasEntreFechas(AF^.FechaInicial,AF^.FechaFinal,FechaCorte,Bisiesto);
      ...
      begin
        ...
        try
         ...
         if FechaPagoI > FechaCorte then
          ...
          begin
            New(AF);
            AF^.FechaInicial := IncDay(FechaInicialMora);
            AF^.FechaFinal := FechaCorte;
            AF^.Anticipado := False;
            AF^.Causado := False;
            AF^.Corrientes := False;
...
//            CalcularFechasMora(FechaCorte,FechaPagoI,FechasLiq);
         // Proceso de Liquidación de Fechas
         TotalInteres := 0;
         for J := 0 to (FechasLiq.Count - 1) do
          begin
